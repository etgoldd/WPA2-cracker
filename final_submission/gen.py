...
def gen_pcaps(
    ...
    @param solution_file_hint: Optional[str]
        A skeleton/solution file to be sent in the 3rd message as a hint to solvers.
    ...
    if anonce is None:
        ...
        print(f"[!] No anonce provided, randomly generating one.")
    if snonce is None:
        snonce = bytes([random.randrange(0, 256) for _ in range(0, 32)])
        print(f"[!] No snonce provided, randomly generating one.")
    if ssid is None:
        ssid = f"{group_name}_wifi"
        print(f"[!] No ssid provided, randomly generated `{ssid}` to be the ssid.")
    if passkey is None:
        ...
        taz = ''.join(random.choices(string.digits, k=8))
        taz += luhn_correct_bit(taz)
        passkey = taz
        MESSAGES[2] += f"The password is my taz, you remember it right? It's {taz[:3]}******, good luck!"
        print(f"[!] No password provided, randomly generated taz as a password: {taz}.")
    elif passkey == "gen_super_secret":
        passkey = f"{group_name}_super_secret_password"
        print(f"[!] `gen_super_secret` provided as a passkey, generated `{passkey}` as the passkey.")
    if flag is None:
        flag = binascii.b2a_hex(hashlib.sha256(group_name.encode('ascii')).digest())[:32].decode('ascii')
        print(f"[!] No flag provided, randomly generated flag is: WPA2-CTF{{{flag}}}")
    if solution_file_hint is not None:
        print(f"[!] Provided a solution file hint")
        MESSAGES[2] += f" Just in case you forgot my taz, here's a cool skeleton\
            I wrote for bruteforcing passwords on WPA2. Don't worry, even if someone is listenning, there's\
             no way they are smart enough to figure out how this works. ```python\n{solution_file_hint}\n```\n"
